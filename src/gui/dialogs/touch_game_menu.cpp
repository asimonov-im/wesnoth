...
REGISTER_DIALOG(touch_game_menu);
...
{
	video_ = &video;
	twindow* ret = build(video, window_id());

	ret->connect_signal<event::SDL_LEFT_BUTTON_UP>(boost::bind(&outside_click_dismiss, ret, _5));

	change_page(0, ret);
	//tbutton* btn = dynamic_cast<tbutton*>(ret->find("button0",false));
	//assert(btn);
	//btn->set_retval(SAVE_GAME);
	//btn->set_label("Save Game");
	return ret;
}
...
{
	window.connect_signal<event::SDL_LEFT_BUTTON_UP>(boost::bind(&outside_click_dismiss, &window, _5));

	change_button(&window, "button0", SAVE_GAME, "Save Game");
	change_button(&window, "button1", LOAD_GAME, "Load Game");
	change_button(&window, "button2", QUIT_GAME, "Quit Game");
	change_button(&window, "button3", OBJECTIVES, "Objectives");
	change_button(&window, "button4", RECALL, "Recall");
	change_button(&window, "button5", RECRUIT, "Recruit");
	change_button(&window, "button6", STATISTICS, "Statistics");
	change_button(&window, "button7", UNIT_LIST, "Unit List");
	change_button(&window, "button8", ENEMY_MOVES, "Enemy Moves");
	change_button(&window, "button9", PREFERENCES, "Preferences");
	change_button(&window, "button10", UNDO, "Undo");
	change_button(&window, "button11", REDO, "Redo");
}
